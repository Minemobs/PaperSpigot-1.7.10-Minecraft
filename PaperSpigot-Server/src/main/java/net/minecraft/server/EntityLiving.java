...
public abstract class EntityLiving extends Entity {
    ...
    protected void aO()
    {
        for (UnmodifiableIterator localUnmodifiableIterator = ImmutableMap.copyOf(this.effects).entrySet().iterator(); localUnmodifiableIterator.hasNext();)
        {
    ...
    {
        addEffect(mobeffect, PotionEffectAddEvent.EffectCause.UNKNOWN);
    }

    public void addEffect(MobEffect mobeffect, PotionEffectAddEvent.EffectCause cause)
    {
        if (d(mobeffect))
        {
            ...
            if (current != null)
            {
                PotionEffectExtendEvent event = new PotionEffectExtendEvent((LivingEntity)getBukkitEntity(), CraftPotionUtils.toBukkit(mobeffect), CraftPotionUtils.toBukkit(current), cause);
                this.world.getServer().getPluginManager().callEvent(event);
                if (event.isCancelled()) {
                    return;
                }
            ...
            {
                PotionEffectAddEvent event = new PotionEffectAddEvent((LivingEntity)getBukkitEntity(), CraftPotionUtils.toBukkit(mobeffect), cause);
                this.world.getServer().getPluginManager().callEvent(event);
                if (event.isCancelled()) {
                    return;
                }
    ...
    public void removeEffect(int i)
    {
        ...
        if (mobeffect != null)
        {
            PotionEffectRemoveEvent event = new PotionEffectRemoveEvent((LivingEntity)getBukkitEntity(), CraftPotionUtils.toBukkit(mobeffect));
            this.world.getServer().getPluginManager().callEvent(event);
            if (event.isCancelled()) {
                return;
            }
